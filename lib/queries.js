...
/**
 * Returns the flow-step is the last step of the flow.
 *
 * [CONSIDERED AUTH. SAFE]
 *
 * @param FS
 ...
 */
export async function isLastStep(FS) {
  const response = await query(`
  PREFIX dct:  <http://purl.org/dc/terms/>
  PREFIX subsidie: <http://data.vlaanderen.be/ns/subsidie#>
  
  SELECT ?step WHERE {
      GRAPH ${sparqlEscapeUri(COMMON_GRAPHS.APPLICATION)} {
        ${sparqlEscapeUri(FS)} dct:references ?step .
        ?step subsidie:Subsidieprocedurestap.type ${sparqlEscapeUri(SUBSIDY_MEASURE_STEP_TYPES.REQUEST)}  .
    }
  }`);
...
export async function generateNotification(subsidyMeasureConsumption, previousStep, currentStep = null) {
  ...
  let query = `
    ...
    INSERT DATA {
      GRAPH ${sparqlEscapeUri(COMMON_GRAPHS.PUBLIC)} {
...
/**
 * Update the consumption-status to the given status.
 *
 * [CONSIDERED AUTH. SAFE]
 *
 * @param consumption
 ...
 * @returns {Promise<*>}
 */
export async function updateConsumptionStatus(consumption, status) {
  return await query(`
  PREFIX adms: <http://www.w3.org/ns/adms#>

  DELETE {
    GRAPH ${sparqlEscapeUri(COMMON_GRAPHS.APPLICATION)} {
      ${sparqlEscapeUri(consumption)} adms:status ?old .
    }
  } WHERE {
    GRAPH ${sparqlEscapeUri(COMMON_GRAPHS.APPLICATION)} {
      ${sparqlEscapeUri(consumption)} adms:status ?old .
  ...
  INSERT DATA {
    GRAPH ${sparqlEscapeUri(COMMON_GRAPHS.APPLICATION)} {
      ${sparqlEscapeUri(consumption)} adms:status ${sparqlEscapeUri(status)} .
    }
  }`);
}